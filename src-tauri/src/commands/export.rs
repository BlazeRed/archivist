...
pub fn export_group(
    ...
    let group = db.get_group(group_id)?.ok_or(AppError::GroupNotFound { id: group_id })?;
    ...
    let dest = Path::new(dest_path).join(&group.name);
    ...
    for image_id in image_ids {
        if let Some(image) = db.get_image(&image_id)? {
            ...
            let dst = dest.join(&image.filename); // dest is already the group subfolder
